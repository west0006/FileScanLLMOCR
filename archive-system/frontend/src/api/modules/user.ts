import { api } from './_base'

export const userApi = {
  list: (params: any) => api.get('/user/', { params }),
  create: (data: any) => api.post('/user/', data),
  get: (id: number) => api.get(`/user/${id}`),
  update: (id: number, data: any) => api.put(`/user/${id}`, data),
  resetPassword: (id: number, pass: string) => api.put(`/user/${id}/password`, null, { params: { new_password: pass } }),
  toggleStatus: (id: number, active: boolean) => api.put(`/user/${id}/status`, null, { params: { is_active: active } }),
  changePassword: (oldPassword: string, newPassword: string) => api.put('/user/me/password', { old_password: oldPassword, new_password: newPassword }),
  batchStatus: (userIds: number[], isActive: boolean) => api.post('/user/batch-status', { user_ids: userIds, is_active: isActive }),
  getTreeAuth: (id: number) => api.get(`/user/${id}/tree-auth`),
  updateTreeAuth: (id: number, nodeIds: string[]) => api.put(`/user/${id}/tree-auth`, { node_ids: nodeIds }),
  listOnline: () => api.get('/user/online'),
  listRoles: () => api.get('/user/roles'),
  createRole: (name: string, description: string) => api.post('/user/roles', null, { params: { name, description } }),
  updatePermissions: (roleId: number, perms: any, dataPerms?: any) => api.put(`/user/roles/${roleId}/permissions`, { permissions: perms, data_permissions: dataPerms }),
  deleteRole: (roleId: number) => api.delete(`/user/roles/${roleId}`),
}
