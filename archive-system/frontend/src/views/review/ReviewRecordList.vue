<template>
  <div class="records-page">
    ...
    <div class="process-banner">
      ...
      以下为AI预审完成的档案记录。可按风险等级、AI建议、年度范围进行筛选。<strong>建议优先查看高风险档案</strong>，通过导出功能批量获取预审结果表格。共
      <strong>{{ total }}</strong> 条记录，当前筛选结果 <strong>{{ filteredCount }}</strong> 条。
    </div>

    <!-- 视图切换 -->
    <div class="view-toggle">
      <button :class="['toggle-btn', { active: reviewView === 'item' }]" @click="reviewView = 'item'">
        <IconSvg name="doc" size="14" /> 按件展示
      </button>
      <button :class="['toggle-btn', { active: reviewView === 'volume' }]" @click="reviewView = 'volume'">
        <IconSvg name="folder" size="14" /> 按卷展示
      </button>
    </div>
    ...
    <div class="filter-bar">
      ...
      <select v-model="yearFromStr" class="filter-select" style="width:90px"
        @change="filters.year_from = yearFromStr ? Number(yearFromStr) : undefined">
        <option value="">起始年</option>
        <option v-for="y in yearOptions" :key="'f' + y" :value="y">{{ y }}</option>
      </select>
      <span class="filter-sep">—</span>
      <select v-model="yearToStr" class="filter-select" style="width:90px"
        @change="filters.year_to = yearToStr ? Number(yearToStr) : undefined">
    ...
    <template v-if="reviewView === 'item'">
      <div class="table-card" v-for="group in groupedRecords" :key="group.dept">
        ...
        <table class="data-table">
          <thead>
            <tr>
              <th style="width:36px"><input type="checkbox" :checked="group.allSelected" @change="toggleGroup(group)"
                  :ref="el => { if (el) (el as HTMLInputElement).indeterminate = group.someSelected && !group.allSelected }" />
              </th>
              <th>档案编号</th>
              <th>题名</th>
              <th style="width:100px">所属案卷</th>
              <th style="width:60px">年度</th>
              <th style="width:140px">风险评分</th>
          ...
          <tbody>
            <tr v-for="row in group.items" :key="row.id" class="clickable">
              <td><input type="checkbox" :checked="selectedIds.includes(row.id)" @click.stop
              ...
              <td>
                <div class="mini-bar">
                  <div class="mini-bar-fill" :class="'mini-bar--' + riskLevelClass(row.risk_level)"
                    :style="{ width: row.risk_score + '%' }"></div><span class="mini-bar-num">{{ row.risk_score }}</span>
                </div>
              </td>
              <td><span class="risk-tag" :class="'risk-tag--' + riskLevelClass(row.risk_level)">{{ row.risk_level
                  }}</span></td>
    ...
    <div class="table-card" v-if="reviewView === 'volume'">
      <table class="data-table">
        <thead>
          <tr>
            <th style="width:36px"><input type="checkbox" :checked="allVolumesSelected" @change="toggleAllVolumes" />
            </th>
            <th>案卷编号</th>
            <th>案卷题名</th>
            <th style="width:70px">年度</th>
            <th style="width:80px">件数</th>
            <th style="width:80px">最高风险</th>
        ...
        <tbody>
          <tr v-for="v in volumeRecords" :key="v.archive_id" class="clickable">
            <td><input type="checkbox" :checked="selectedVolumes.includes(v.archive_id)" @click.stop
                @change="toggleVolume(v.archive_id)" /></td>
            <td class="mono" @dblclick="showVolumeDetail(v)">{{ v.archive_id }}</td>
            <td class="title-cell" @dblclick="showVolumeDetail(v)">{{ v.title }}</td>
            <td>{{ v.year }}</td>
            <td>{{ v.item_count || '-' }}</td>
            <td><span class="risk-tag" :class="'risk-tag--' + riskLevelClass(v.max_risk)">{{ v.max_risk || '-' }}</span>
            </td>
            <td>{{ v.suggestion || '-' }}</td>
            <td>
          ...
          <tr v-if="!volumeRecords.length">
            <td colspan="7" class="table-empty">暂无案卷数据</td>
          </tr>
        </tbody>
      </table>
    </div>
    ...
    <el-pagination v-if="total > pageSize" class="pager" background layout="prev, pager, next, sizes" :total="total"
      ...
      @current-change="(p: number) => { page = p; fetchRecords() }" @size-change="(s: number) => { pageSize = s; fetchRecords() }" />
    ...
    <div class="export-cards">
      ...
      <div class="export-card">
        <h4>
          <IconSvg name="pkg" size="15" /> 导出档案原文压缩包
        </h4>
      ...
      <div class="modal-card">
        ...
        <div class="modal-body">
          <dl class="detail-grid">
            ...
            <div>
              <dt>风险评分</dt>
              <dd class="text-accent font-semibold">{{ selected.risk_score }}</dd>
            </div>
            <div>
              <dt>风险等级</dt>
            ...
            <div class="span-2">
              <dt>AI 建议</dt>
              <dd class="font-semibold">{{ selected.suggestion }}</dd>
            </div>
            <div class="span-2">
              <dt>建议理由</dt>
            ...
            <div>
              <dt>置信度</dt>
              <dd>{{ ((selected.confidence || 0) * 100).toFixed(0) }}%</dd>
            </div>
            <div>
              <dt>耗时</dt>
              <dd>{{ selected.processing_time_ms || '—' }}ms</dd>
            </div>
          </dl>
        </div>
      </div>
...
.dept-header {
  padding: 10px 16px;
  background: linear-gradient(90deg, var(--c-bg), var(--c-surface));
  border-bottom: 1px solid var(--c-border);
  display: flex;
  align-items: center;
  justify-content: space-between
