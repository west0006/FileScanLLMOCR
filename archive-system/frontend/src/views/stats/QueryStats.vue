<template>
  <div class="stats-page">
    <div v-if="hasError" class="error-banner">
      <IconSvg name="warn" size="14" /> {{ errorMsg }}
    </div>
    ...
    <div class="charts-row">
      <div class="chart-card">
        ...
        <div v-if="!typeHasData" class="chart-empty">暂无数据</div>
      </div>
      <div class="chart-card">
        <h3>按用户统计 (Top 10)</h3>
        <div ref="userChartRef" class="chart-box"></div>
        <div v-if="!userHasData" class="chart-empty">暂无数据</div>
      </div>
    </div>
    <div class="charts-row">
    ...
    <div class="card">
      <div class="card-head">
        ...
        <button class="btn-sm" @click="showExportOptions = true">
          <IconSvg name="download" size="14" /> 导出报表
        </button>
      </div>
      <div class="filter-bar">
        <select v-model="userFilter.role" class="filter-input-sm">
          <option value="">全部角色</option>
          <option>system_admin</option>
          <option>archive_admin</option>
          <option>reviewer</option>
        </select>
        <select v-model="userFilter.period" class="filter-input-sm">
      ...
      <table class="data-table">
        ...
        <tbody>
          <tr v-for="(u, i) in userRanking" :key="u.username">
            ...
            <td>{{ u.download || 0 }}</td>
            <td>{{ u.print || 0 }}</td>
            <td><strong class="text-accent">{{
              (u.search || 0) + (u.view_entry || 0) + (u.view_file || 0) + (u.download || 0) + (u.print||0) }}</strong></td>
          </tr>
          <tr v-if="!userRanking.length">
        ...
        <tbody>
          <tr v-for="m in methodDetail" :key="m.type">
            ...
            <td>
              <div class="pct-row">
                <div class="pct-bar">
                  <div class="pct-fill" :style="{ width: m.pct + '%' }"></div>
                </div><span class="pct-num">{{ m.pct }}%</span>
              </div>
            </td>
            <td>{{ m.year_count }}</td>
            <td>{{ m.trend === 'up' ? '↑ 上升' : m.trend === 'down' ? '↓ 下降' : '→ 平稳' }}</td>
          </tr>
          <tr v-if="!methodDetail.length">
    ...
    <div v-if="showExportOptions" class="modal-overlay" @click.self="showExportOptions = false">
      <div class="modal-card" style="width:400px">
        <div class="modal-head">
          <h3>自定义导出</h3><button class="modal-close" @click="showExportOptions = false"><svg width="18" height="18"
        ...
        <div class="modal-body">
          ...
          <div class="form-group"><label>导出字段</label>
            <label v-for="f in exportFields" :key="f.key" class="perm-item"><input type="checkbox"
                v-model="f.selected" /><span class="perm-label">{{ f.label }}</span></label>
          </div>
