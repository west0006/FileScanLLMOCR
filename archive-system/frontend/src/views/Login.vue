      ...
      <div class="login-brand">
        <div class="brand-icon">
          <svg width="32" height="32" viewBox="0 0 28 28" fill="none">
            ...
            <path d="M7 9h14M7 14h10M7 19h6" stroke="#fff" stroke-width="2" stroke-linecap="round" />
          </svg>
        </div>
        <h1>档案智能查询与开放审核系统</h1>
        <p>中南财经政法大学</p>
      ...
      <div class="login-form">
        ...
        <div class="field-group">
          ...
          <div class="captcha-row">
            <input v-model="captchaInput" class="field-input captcha-input" placeholder="请输入验证码" maxlength="4"
              @keyup.enter="handleLogin" />
            <div class="captcha-box" @click="genCaptcha">{{ captchaText }}</div>
          </div>
        </div>
        <div class="role-row">
          <button v-for="r in roles" :key="r.key" :class="['role-btn', { active: selectedRole === r.key }]"
            @click="selectedRole = r.key">{{ r.label }}</button>
        </div>
