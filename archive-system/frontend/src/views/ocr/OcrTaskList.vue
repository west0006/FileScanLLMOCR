<template>
  <div class="page">
    ...
    <div class="quality-bar" v-if="quality.total">
      <span>
        <IconSvg name="chart" size="15" /> 已处理 {{ quality.total }} 件 | 平均准确率 {{
          (quality.overall_accuracy * 100).toFixed(1) }}%
      </span>
      <span v-if="quality.low_confidence_count" style="color:var(--c-warning)"> |
        <IconSvg name="warn" size="14" /> 低置信度 {{ quality.low_confidence_count }} 件
      </span>
      <span v-if="quality.failed_count" style="color:var(--c-danger)"> |
        <IconSvg name="warn" size="12" /> 失败 {{ quality.failed_count }} 件
      </span>
    </div>
        ...
        <tbody>
          <tr v-for="t in tasks" :key="t.id">
            ...
            <td>
              ...
              <button v-if="t.status === 'pending'" class="btn-sm" style="margin-left:4px"
                @click="handleAction(t, 'start')">启动</button>
              <button v-if="t.status === 'running'" class="btn-sm" style="margin-left:4px"
                @click="handleAction(t, 'pause')">暂停</button>
              <button v-if="t.status === 'paused'" class="btn-sm" style="margin-left:4px"
                @click="handleAction(t, 'resume')">恢复</button>
              <button v-if="t.status === 'running' || t.status === 'pending' || t.status === 'paused'" class="btn-sm"
                style="margin-left:4px" @click="handleAction(t, 'cancel')">取消</button>
            </td>
          </tr>
          <tr v-if="tasks.length === 0">
            <td colspan="7" class="table-empty">暂无 OCR 任务，点击右上角「创建任务」开始</td>
          </tr>
        </tbody>
      </table>
    </div>
    ...
    <div v-if="showCreate" class="modal-overlay" @click.self="showCreate = false">
      <div class="modal-card">
        <div class="modal-head">
          <h3>创建 OCR 任务</h3><button class="modal-close" @click="showCreate = false"><svg width="18" height="18"
        ...
        <div class="modal-body">
          <div class="form-group"><label>任务名称 *</label><input class="field-input" v-model="cf.task_name"
              placeholder="如: 1996年行政档案批量OCR" /></div>
          <div class="form-row">
            <div class="form-group" style="flex:1"><label>起始年度</label><input class="field-input"
                v-model.number="cf.year_from" type="number" placeholder="如 1996" /></div>
            <div class="form-group" style="flex:1"><label>截止年度</label><input class="field-input"
                v-model.number="cf.year_to" type="number" placeholder="如 2000" /></div>
          </div>
          <div class="form-row">
            <div class="form-group" style="flex:1"><label>档案门类</label><select class="field-input" v-model="cf.category">
            ...
            <div class="form-group" style="flex:1"><label>优先级</label><select class="field-input"
                v-model.number="cf.priority">
                <option :value="0">普通</option>
                <option :value="1">高</option>
                <option :value="2">紧急</option>
              </select></div>
          </div>
          <div class="form-group"><label>全宗号</label><input class="field-input" v-model="cf.fonds_id"
              placeholder="如 XZ / DQ / JX（可留空）" /></div>
    ...
    <div v-if="showDetail" class="modal-overlay" @click.self="showDetail = false">
      <div class="modal-card" style="width:560px">
        <div class="modal-head">
          <h3>任务详情 — {{ detailTask?.task_name }}</h3><button class="modal-close" @click="showDetail = false"><svg
        ...
        <div class="modal-body">
          <dl class="detail-grid">
            ...
            <div>
              <dt>状态</dt>
              <dd><span class="risk-tag" :class="'risk-tag--' + barClass(detailTask?.status)">{{
                statusLabel(detailTask?.status) }}</span></dd>
            </div>
            <div>
              <dt>总页数</dt>
              <dd>{{ detailTask?.total_pages || '—' }}</dd>
            </div>
            <div>
              <dt>已处理</dt>
              <dd>{{ detailTask?.processed_pages || 0 }}</dd>
            </div>
            <div>
              <dt>失败数</dt>
              <dd style="color:var(--c-danger)">{{ detailTask?.failed_pages || 0 }}</dd>
            </div>
            <div>
              <dt>剩余</dt>
              <dd>{{ (detailTask?.total_pages || 0) - (detailTask?.processed_pages || 0) }}</dd>
            </div>
            <div>
              <dt>优先级</dt>
              <dd>{{ priLabel(detailTask?.priority) }}</dd>
            </div>
            <div>
              <dt>创建时间</dt>
              <dd>{{ detailTask?.created_at?.substring(0, 19) }}</dd>
            </div>
            <div class="span-2">
              <dt>筛选条件</dt>
              <dd class="text-sm text-muted">{{ detailTask?.filter_criteria ? JSON.stringify(detailTask.filter_criteria)
                : '无' }}</dd>
            </div>
          </dl>
        </div>
