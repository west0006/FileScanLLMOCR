<template>
  <div class="page">
    <div class="page-head">
      <h2>操作日志</h2><button class="btn-export" @click="handleExport"><svg width="14" height="14" viewBox="0 0 24 24"
    ...
    <div class="log-toolbar">
      <div class="log-tabs">
        <button v-for="tab in logTabs" :key="tab.key" :class="['log-tab', { active: logTab === tab.key }]"
          @click="logTab = tab.key; fetchLogs()">
          {{ tab.label }} <span class="tab-badge">{{ tab.count }}</span>
        </button>
      </div>
      <div class="stats-card">
        <span>
          <IconSvg name="chart" size="15" /> 今日操作 <strong>{{ logStats.total }}</strong>
        </span>
        <span class="stats-sep">|</span>
        <span>
          <IconSvg name="warn" size="14" /> 失败 <strong style="color:var(--c-danger)">{{ logStats.failed }}</strong>
        </span>
        <span class="stats-sep">|</span>
        <span>
          <IconSvg name="pkg" size="15" /> 保留 <strong>{{ logStats.retention }}</strong> 天
        </span>
      </div>
    </div>
    <div class="log-stat-grid">
      <div class="log-stat-card">
        <div class="lsc-label">全部日志</div>
        <div class="lsc-value">{{ logStats.allCount }}</div>
      </div>
      <div class="log-stat-card">
        <div class="lsc-label">用户登录</div>
        <div class="lsc-value">{{ logStats.loginCount }}</div>
      </div>
      <div class="log-stat-card">
        <div class="lsc-label">功能访问</div>
        <div class="lsc-value">{{ logStats.accessCount }}</div>
      </div>
      <div class="log-stat-card">
        <div class="lsc-label">功能操作</div>
        <div class="lsc-value">{{ logStats.operationCount }}</div>
      </div>
    </div>
      ...
      <table class="data-table">
        ...
        <tbody>
          <tr v-for="row in items" :key="row.id">
            ...
            <td><span class="risk-tag" :class="row.result === 'success' ? 'risk-tag--low' : 'risk-tag--high'">{{
              row.result === 'success' ? '成功':'失败' }}</span></td>
          </tr>
    ...
    <el-pagination v-if="total > pageSize" class="pager" background layout="prev, pager, next, sizes" :total="total"
      ...
      @current-change="(p: number) => { page = p; fetchLogs() }" @size-change="(s: number) => { pageSize = s; fetchLogs() }" />
  </div>
</template>
